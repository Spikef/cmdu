...
参数默认是必需的，也可以显式地声明为`[可选的]`还是`<必需的>`。如果定义了一个必需输入的参数而用户忘记给定值，则将在命令行输出错误提示。
...
一个命令选项包含了定义(opt)，名称(name)，描述(description)，类型，转换函数和默认值。
...
选项默认为可选的，当选项的值不是布尔值类型时，可以显式地声明为`[可选的]`还是`<必需的>`。
...
使用`.action(handle)`方法传入一个回调函数或回调函数模块的路径(相对于定义该命令的脚本文件路径)，用于响应用户输入该命令时的操作。
