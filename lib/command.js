...
var setTitleStyle = function(text) {
    return setStyle(text, 'bold', 'underline');
};
...
Command.prototype.alias = function (name) {
    ...
    this._alias = (this._alias || []).concat(name);
};
...
Command.prototype.showHelp = function (callback) {
    ...
    if (this.description) {
        message.push('  ' + setTitleStyle(help.title.about));
        message.push('');
        message.push('    ' + setStyle.italic(this.description));
        message.push('');
    }

    message.push('  ' + setTitleStyle(help.title.usage));
    message.push('');

    if (this.name === '*') {
        if (this.origin !== '*') {
            message.push('    ' + cmd + this.origin.replace('*', '') + ' [' + help.message.options + ']');
        } else if (this.subs.length) {
            message.push('    ' + cmd + ' [command] [' + help.message.options + ']');
        } else {
            message.push('    ' + cmd+ ' [' + help.message.options + ']')
        }
    } else {
        var names = [this.name].concat(this._alias || []);
        var remain = this.origin.replace(this.name, names.join('|'));
        message.push('    ' + cmd + ' ' + remain + ' [' + help.message.options + ']');
    }
    ...
    if (this.subs.length) {
        message.push('');
        message.push('  ' + setTitleStyle(help.title.argument));
        message.push('');
    ...
    if (this.cmds.length) {
        message.push('');
        message.push('  ' + setTitleStyle(help.title.command));
        message.push('');
    ...
    if (this.options.length) {
        message.push('');
        message.push('  ' + setTitleStyle(help.title.option));
        message.push('');
